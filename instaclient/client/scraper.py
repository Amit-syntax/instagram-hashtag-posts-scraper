...
class Scraper(Component):
    TYPES = [InstaBaseObject.GRAPH_IMAGE,
             InstaBaseObject.GRAPH_VIDEO, InstaBaseObject.GRAPH_SIDECAR]
    ...
    def get_notifications(self: 'InstaClient', types: list = None, count: int = None) -> Optional[List[Notification]]:
        ...
        if types is None or len(types) == 0:
            types = [InstaBaseObject.GRAPH_FOLLOW, InstaBaseObject.GRAPH_LIKE,
                     InstaBaseObject.GRAPTH_TAGGED, InstaBaseObject.GRAPH_COMMENT, InstaBaseObject.GRAPH_MENTION]
        else:
            for type in types:
    ...
    def get_profile(self: 'InstaClient', username: str, context: bool = False) -> Optional['Profile']:
        ...
        data = self._request(GraphUrls.GRAPH_USER.format(
            username), use_driver=context)
    ...
    def get_post(self: 'InstaClient', shortcode: str, context: bool = False) -> Optional['Post']:
        ...
        data = self._request(GraphUrls.GRAPH_POST.format(
            shortcode), use_driver=context)
        if not data:
            raise InvalidInstaRequestError(
                GraphUrls.GRAPH_POST.format(shortcode))
        ...
        try:
            ...
            if location:
                location = Location(
                    ...
                    address=Address(
                        location['address_json'],) if location['address_json'] else None
                )
    ...
    def get_user_posts(self: 'InstaClient', username: str, count: Optional[int] = 30, deep_scrape: Optional[bool] = False, callback_frequency: int = 100, callback=None, **callback_args) -> Optional[Union[List[str], List[Profile]]]:
        ...
        try:
            while len(shortcodes) < count:
                ...
                print("got -[", len(shortcodes), "] urls to scrape")
                loop = time.time()  # TODO
                LOGGER.debug(
                    f'Starting Post Scrape Loop. Posts: {len(shortcodes)}')

                scraped_count = len(shortcodes)
                divs = self._find_element(EC.presence_of_all_elements_located(
                    (By.XPATH, Paths.SHORTCODE_DIV)), wait_time=2)
        ...
        if not deep_scrape:
            ...
            for index, shortcode in enumerate(shortcodes):
                LOGGER.debug(
                    f'Deep scraped {index} posts out of {len(shortcodes)}')
    ...
    def get_followers(self: 'InstaClient', user: str, count: int, use_api: bool = True, deep_scrape: Optional[bool] = False, end_cursor: str = None, callback_frequency: int = 100, callback=None, **callback_args) -> Optional[Union[List[Profile], List[str]]]:
        ...
        if not use_api:
            ...
            # Nav Followers Page
            followers_btn: WebElement = self._find_element(EC.presence_of_element_located(
                (By.XPATH, Paths.FOLLOWERS_BTN)), url=ClientUrls.NAV_USER.format(user))
            # Click followers btn
            ...
            try:
                while len(followers) < count:
                    loop = time.time()  # TODO
                    LOGGER.debug(
                        f'Starting Scrape Loop. Followers: {len(followers)}')

                    scraped_count = len(followers)
                    divs = self._find_element(EC.presence_of_all_elements_located(
                        (By.XPATH, Paths.FOLLOWER_USER_DIV)), wait_time=2)
                        ...
                        try:
                            ...
                            if username not in followers and username not in ('Follow',) and len(followers) < count:
                                ...
                                if len(followers) % callback_frequency == 0:
                                    if callable(callback):
                                        LOGGER.debug('Called Callback')
                                        callback(scraped=followers,
                                                 **callback_args)
                                    else:
                                        LOGGER.info(
                                            f'Scraped {len(followers)} followers so far...')
            ...
            if not cursor:
                request = GraphUrls.GRAPH_FIRST_FOLLOWERS.format(
                    QUERY_HASH=QueryHashes.FOLLOWERS_HASH, ID=profile.id)
            else:
                request = GraphUrls.GRAPH_CURSOR_FOLLOWERS.format(
                    QUERY_HASH=QueryHashes.FOLLOWERS_HASH, ID=profile.id, END_CURSOR=cursor)
            requests = 1
            looping = True
            while looping:
                ...
                if not status == 'ok':
                    if result.get('message') == 'rate limited':
                        LOGGER.exception(
                            'Rate limit reached. Stopping scrape.')
                        break
                    else:
                        LOGGER.exception(
                            f'The request with cursor {cursor} failed')
                        break
        ...
        if not deep_scrape:
            ...
            for index, follower in enumerate(followers):
                try:
                    ...
                    LOGGER.debug(
                        f'Deep scraped {index} profiles out of {len(followers)}')
                except:
    ...
    def get_following(self: 'InstaClient', user: str, count: int, use_api: bool = True, deep_scrape: Optional[bool] = False, end_cursor: str = None, callback_frequency: int = 100, callback=None, **callback_args) -> Union[Optional[Union[List[Profile], List[str]]], Optional[str]]:
        ...
        if not use_api:
            ...
            # Nav Followers Page
            following_btn: WebElement = self._find_element(EC.presence_of_element_located(
                (By.XPATH, Paths.FOLLOWED_BTN)), url=ClientUrls.NAV_USER.format(user))
            # Click followers btn
            ...
            try:
                while len(following) < count:
                    loop = time.time()  # TODO
                    LOGGER.debug(
                        f'Starting Scrape Loop. Following: {len(following)}')
                        ...
                        try:
                            ...
                            if username not in following and username not in ('Follow',) and len(following) < count:
                                ...
                                if len(following) % callback_frequency == 0:
                                    if callable(callback):
                                        LOGGER.debug('Called Callback')
                                        callback(scraped=following,
                                                 **callback_args)
                                    else:
                                        LOGGER.info(
                                            f'Scraped {len(following)} following so far...')
            ...
            if not end_cursor:
                request = GraphUrls.GRAPH_FIRST_FOLLOWING.format(
                    QUERY_HASH=QueryHashes.FOLLOWING_HASH, ID=profile.id)
            else:
                request = GraphUrls.GRAPH_CURSOR_FOLLOWING.format(
                    QUERY_HASH=QueryHashes.FOLLOWING_HASH, ID=profile.id, END_CURSOR=end_cursor)
            requests = 1
            looping = True
            while looping:
                result = self._request(request, use_driver=True)
                ...
                if not page_info or not page_info.get('end_cursor'):
                    ...
                    cursor = page_info['end_cursor'].replace('==', '')
                    request = GraphUrls.GRAPH_CURSOR_FOLLOWING.format(
                        QUERY_HASH=QueryHashes.FOLLOWING_HASH, ID=profile.id, END_CURSOR=cursor)
            ...
            for index, follower in enumerate(following):
                try:
                    if isinstance(follower, Profile):
                        ...
                        profiles.append(self.get_profile(follower))
                    LOGGER.debug(
                        f'Deep scraped {index} profiles out of {len(following)}')
                except:
                    failed.append(follower)
            LOGGER.warning(f'Failed: {len(failed)}')
            return profiles, cursor
    ...
    def get_hashtag_posts(self: 'InstaClient', tag: str, count: int, deep_scrape: Optional[bool] = False, callback_frequency: int = 100, callback=None, **callback_args) -> Optional[Union[List[str], List[Post]]]:
        ...
        try:
            while len(shortcodes) < count:
                ...
                LOGGER.debug('Scroll')
                self.scroll(mode=self.END_PAGE_SCROLL, times=2, interval=2)
        except Exception as error:
            LOGGER.error('ERROR IN SCRAPING POSTS', exc_info=error)
            return shortcodes
        ...
        for item in users_data:
            try:
                ...
                users.append(Profile(
                    ...
                    followed_by_viewer=None if not friendship_status else friendship_status.get(
                        'is_following'),
                    has_requested_viewer=None if not friendship_status else friendship_status.get(
                        'incoming_request'),
                    requested_by_viewer=None if not friendship_status else friendship_status.get(
                        'outgoing_request'),
    ...
    def _request(self: 'InstaClient', url: str, use_driver: bool = False) -> Optional[dict]:
        ...
        if use_driver:
            try:
                self.driver.get(url)
                element: WebElement = self._find_element(EC.presence_of_element_located(
                    (By.XPATH, Paths.QUERY_ELEMENT)), wait_time=2, retry=False)
                source = element.text
