...
class Component:
    ...
    def connect(self: 'InstaClient', login=False, retries=0, func=None):
        ...
        """
        LOGGER.debug(
            'INSTACLIENT: Initiating Driver | attempt {} | func: {}'.format(retries, func))
        try:
            if self.driver_type == self.CHROMEDRIVER:
                if self.host_type == self.WEB_SERVER:
                    ...
                    if self.proxy:
                        chrome_options.add_argument(
                            f'--proxy-server={self.proxy}:{self.port}s')
                    self.driver = webdriver.Chrome(executable_path=os.environ.get(
                        "CHROMEDRIVER_PATH"), chrome_options=chrome_options, service_log_path=os.devnull)
                    ...
                    chrome_options.add_experimental_option(
                        "mobileEmulation", mobile_emulation)
                    chrome_options.add_argument(
                        "--headless") if self.localhost_headless else None
                    chrome_options.add_experimental_option(
                        "excludeSwitches", ['enable-automation'])
                    chrome_options.add_argument("--disable-dev-shm-usage")
                    chrome_options.add_argument('--log-level=4')
                    ...
                    self.driver = HiddenChromeWebDriver(
                        executable_path=self.driver_path, chrome_options=chrome_options, service_log_path=os.devnull)
                else:
            ...
            if retries < 2:
                LOGGER.debug(
                    'INSTACLIENT: Error when initiating driver... Trying again')
        ...
        if login:
            try:
                ...
                raise InstaClientError(
                    message='Tried logging in when initiating driver, but username and password are not defined.')
        return self
    ...
    def _find_element(self: 'InstaClient', expectation, url: str = None, wait_time: int = 5, retry=True, attempt=0):
        ...
        try:
            ...
            if ClientUrls.CHALLENGE_URL in self.driver.current_url:
                self.logger.warning(
                    'Account Blocked. Manual action is required.')
                ...
                if attempt == 0:
                    ...
                    if ClientUrls.LOGIN_URL in self.driver.current_url:
                        if url is not None:
                            ...
                            LOGGER.exception(
                                'The element with locator {} was not found'.format(expectation.locator))
                            raise NoSuchElementException()
    ...
    def _dismiss_cookies(self):
        if self._check_existence(EC.presence_of_element_located((By.XPATH, Paths.ACCEPT_COOKIES)), wait_time=2.5):
            accept_btn = self._find_element(
                EC.presence_of_element_located((By.XPATH, Paths.ACCEPT_COOKIES)))
            self._press_button(accept_btn)
        LOGGER.debug('Dismissed Cookies')

    def _dismiss_useapp_bar(self, wait_time=1.5):
        dismiss_bar = self._check_existence(EC.presence_of_element_located(
            (By.XPATH, Paths.USE_APP_BAR)), wait_time=wait_time)
        if dismiss_bar:
            LOGGER.debug('Dismissed Use App Bar')
            dismiss = self._find_element(
                EC.presence_of_element_located((By.XPATH, Paths.USE_APP_BAR)))
    ...
    def _dismiss_dialogue(self: 'InstaClient', wait_time: float = 1.5):
        ...
        try:
            if self._check_existence(EC.presence_of_element_located((By.XPATH, Paths.DIALOGUE)), wait_time=wait_time):
                dialogue = self._find_element(EC.presence_of_element_located(
                    (By.XPATH, Paths.NOT_NOW_BTN)), wait_time=wait_time)
    ...
    def _detect_restriction(self):
        ...
        """
        restriction = self._check_existence(EC.presence_of_element_located(
            (By.XPATH, Paths.RESTRICTION_DIALOG)), wait_time=1.2)
        if restriction:
            LOGGER.warn('INSTACLIENT: WARNING: ACCOUNT HAS BEEN RESTRICTED')
            buttons = self._find_element(EC.presence_of_element_located(
                (By.XPATH, Paths.RESTRICTION_DIALOGUE_BTNS)), retry=False)
            buttons.click()
            time.sleep(randrange(2, 4))
            raise RestrictedAccountError(None)

        block = self._check_existence(EC.presence_of_element_located(
            (By.XPATH, Paths.BLOCK_DIV)), wait_time=1.2)
        if block:
            LOGGER.warn(
                'INSTACLIENT: WARNING: ACCOUNT HAS BEEN BLOCKED - Log in Manually')
